...
public final class OpinionAggregator {
    ...
    public OpinionAggregationResult aggregate(InfluenceNetwork network, AgentState targetAgent) {
        ...
        if (inbound.isEmpty()) {
            return new OpinionAggregationResult(targetAgent.getOpinionValue(), 0.0, false, null, false, List.of());
        }
        ...
        if (relayedAsIs) {
            ...
            return new OpinionAggregationResult(
                    relayedOpinion, Math.abs(incomingInfluence), true, relayOrigin, true, List.copyOf(contributors));
        }
