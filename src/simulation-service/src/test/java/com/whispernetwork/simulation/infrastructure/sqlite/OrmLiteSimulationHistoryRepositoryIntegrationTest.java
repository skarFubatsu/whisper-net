...
class OrmLiteSimulationHistoryRepositoryIntegrationTest {
  ...
  void shouldRecordEventsAndFilterByTypeAndRun() {
    ...
    SimulationEvent tickCompleted = new SimulationEvent.SimulationTickCompleted(
        ...
        1,
        1,
        List.of(new AgentOpinionUpdate(
            "agent-1",
            0.1,
            0.3,
            0.2,
    ...
    SimulationEvent failed = new SimulationEvent.SimulationFailed(
        runId,
        "network-a",
        "boom",
        t3);
    ...
    try (OrmLiteSimulationHistoryRepository repository = new OrmLiteSimulationHistoryRepository(jdbcUrl)) {
      ...
      TimelineQuery tickOnlyQuery = new TimelineQuery(
          runId,
          "network-a",
          List.of("SIMULATION_TICK_COMPLETED"),
          new TimelineWindow(null, null, 50, 0L));
      ...
      TimelineQuery orderedQuery = new TimelineQuery(
          runId,
          "network-a",
          List.of(),
          new TimelineWindow(null, null, 50, 0L));
  ...
  void shouldApplyTimeWindowAndPagination() {
    ...
    try (OrmLiteSimulationHistoryRepository repository = new OrmLiteSimulationHistoryRepository(jdbcUrl)) {
      ...
      TimelineQuery windowed = new TimelineQuery(
          runId,
          "network-a",
          List.of(),
          new TimelineWindow(1_500L, 3_000L, 10, 0L));
      ...
      TimelineQuery paged = new TimelineQuery(
          runId,
          "network-a",
          List.of(),
          new TimelineWindow(null, null, 1, 1L));
